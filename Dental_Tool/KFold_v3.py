...
def split_K_fold(tooth_num, argscale, K_num=5):
        ...
        ranges = [ round(part*i) for i in range(K_num) ]
        ranges += [ tooth_num ]
        range_num = len(ranges) - 1
        
        #         ranges = [ round(part), round(part*2), round(part*3), round(part*4), tooth_num]
        range_set = np.zeros((range_num, 2), int)
